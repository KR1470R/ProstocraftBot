...
function getCaptchaAnswer(bot) {
	bot._client.on("map", (data) => {
		...
		if(bot.inventory.findInventoryItem(358) == null) { 
			emitter.emit("captcha")
			return;
		}
		if(bot.inventory.findInventoryItem(358).slot != 36) {
			emitter.emit("captcha")
			return;
		};
		...
		is_logged = true
		img.writeImage(`${image_path}/captcha.png`, function (err) {
			if (err) throw err;
		});
		console.log("Captcha image instance generated.")
		getCaptchaAnswer(bot)
		rl.question('Enter captcha from image! ', (answer) => {
...
exports.loginBot = function(bot,image_path,password) {
	bot.on("login", async => {
		getCaptchaAnswer(bot);
		emitter.on("captcha", async => {
			bot.chat(`/register ${password} ${password}`)
			bot.chat(`/login ${password}`)
			bot.emit("auth")
		})
		bot.on("kicked", (reason) => {
			console.log(`Bot kicked with reason: ${reason}`)
		});
	})
}
